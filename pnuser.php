...
function iw_bookings_user_anulla($arg){
	...
	if (empty($confirm)){
		...
		if ($bookingInfo[count]>0){ LogUtil::registerError(__('This booking belongs to a set of simultaneous bookings', $dom));}
		if ($bookingInfo['temp']==1){ LogUtil::registerError(__('Warning, this is a preferential booking.', $dom));}
...
function iw_bookings_user_reserva($args){
	...
	if(is_null($user)) $user = pnUserGetVar('uid');
	...
	if (!SecurityUtil::checkPermission('iw_bookings::', '::', ACCESS_ADMIN)){
    	...
    	if ($maxWeeks != 0){
    		...
    		if ($alert) $msg = __('Bookings can only be made for the following ', $dom). $maxWeeks.__(' week(s). Haven\'t made all the requested bookings.', $dom);	
    	}
	}
	...
	foreach ($bookingDates as $bd){
	    ...
	    if (count($bookings) != 0) $reserved[] = $bookings;		       
	}
...
function iw_bookings_user_espais($args){
    ...
    foreach ($items as $item) {
		if ($item['active']){
			...
			if (pnModGetVar('iw_bookings', 'showcolors')) { $color = $item['color'];} else {$color = "";}
